...
class TestProductModel(unittest.TestCase):
    ...
    def test_deserialize_valid_name(self):
        """It should deserialize valid data with a name"""
        product = Product()
        data = {
            "name": "Test Product",
    ...
    def test_deserialize_missing_name(self):
        """It should raise an error when name is missing"""
        product = Product()
        data = {
            "description": "A test description",
            "price": "19.99",
    ...
    def test_deserialize_invalid_name_type(self):
        """It should raise an error when name is not a string"""
        product = Product()
        data = {
            "name": 1234,  # Invalid type
            "description": "A test description",
            "price": "19.99",
            "available": True,
            "category": "TOOLS"
    ...
    def test_app_initialization(self):
        """It should initialize the Flask app correctly"""
    ...
    def test_deserialize_invalid_field_type(self):
        """It should raise a DataValidationError when a field has an invalid type"""
        product = Product()
        data = {
        ...
        with self.assertRaises(DataValidationError) as context:
            product.deserialize(data)
        self.assertIn("Invalid type for string [name]", str(context.exception))

    def test_deserialize_invalid_enum_value(self):
        """It should raise a DataValidationError when category is invalid"""
        product = Product()
        data = {
            "name": "Test Product",
            "description": "A test description",
    ...
    def test_find_by_price_string(self):
        ...
        product1 = ProductFactory(price=19.99)
        product2 = ProductFactory(price=29.99)
        product1.create()
        product2.create()

        # Use price as a string
        found_products = Product.find_by_price("19.99")
        self.assertEqual(len(found_products), 1)
        self.assertEqual(float(found_products[0].price), 19.99)

    def test_find_by_price_decimal(self):
        """It should find products by price when price is a Decimal"""
        product1 = ProductFactory(price=19.99)
        product2 = ProductFactory(price=29.99)
        product1.create()
        product2.create()

        # Use price as a Decimal
